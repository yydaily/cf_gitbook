...
显然，对于多出来的任意标号，他的取值范围只能是 $$$[0, n-1]$$$，所以，不妨将初始状态的hash值，加上这些枚举构成一个序列，然后和所有节点的标号做一次取交集，交集中所有的点，都是好节点。只需要手动给所有hash值一个人为的顺序，就可以用双指针优化，将交集操作优化到 $$$O(n)$$$，但是这里由于需要排序，所以总复杂度为 $$$O(n*log(n))$$$
